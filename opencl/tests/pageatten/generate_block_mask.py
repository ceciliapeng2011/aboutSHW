...
def count_false_percentage(mask, is_stacked_by_trunk=False):
    # mask is in shape [num_trunks, H, NQ_per_trunk, NL] or [num_layers, H, NQ_per_seq, NL]
    if is_stacked_by_trunk:
        num_trunks, H, NQ_per_trunk, NL = mask.shape
        mask = mask.transpose(0, 1).contiguous().view(1, H, num_trunks * NQ_per_trunk, NL)  # [H, NQ, NL]
